...
const RepositoryCard: React.FC<Props> = ({repository}) => {
  const description =
    repository.description?.replace(/:\w+:/g, (match) => {
      ...
      return emoji
        ? `<span><img src="${emoji}" class="emoji" alt="${name}"></span>`
        : match;
    }) ?? 'No description';
  ...
  return (
    <div className={styles.repo_card}>
      ...
      <div
        ...
        style={{fontStyle: !repository.description ? 'italic' : 'normal'}}>
        {description}
      </div>
      ...
      <div className={styles.repo_card__metadata}>
        {repository.language && (
          <div className={styles.repo_card__metadata__language}>
            <a
              href={
                repository.language in colors
                  ? colors[repository.language].url
                  : ''
              }>
              <span
                style={{
                  backgroundColor:
                    repository.language in colors
                      ? colors[repository.language].color
                      : '',
                }}></span>
