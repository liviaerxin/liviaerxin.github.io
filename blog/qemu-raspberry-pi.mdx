...
In this blog, **QEMU** is employed to emulate **Raspberry Pi 3/4** in mac M1 host(it's also supposed to work in Windows/Linux). I will demonstrate **two** different ways to emulate **Raspberry Pi 3** and **Raspberry Pi 4** in respect. These two ways are different by using different **QEMU machines** as you would like to use:
...
1. Use `ubuntu-22.04-preinstalled-server-arm64+raspi.img`, of which the default user is `ubuntu` and password is `ubuntu`.

```sh
kernel="$PWD/ubuntu-22.04-preinstalled-server-arm64+raspi-boot/vmlinuz"
initrd="$PWD/ubuntu-22.04-preinstalled-server-arm64+raspi-boot/initrd.img"
img="$PWD/ubuntu-22.04-preinstalled-server-arm64+raspi.img"
```
...
- `-append`
  - `root=/dev/sda2`: the second partition of the `ubuntu-22.04-preinstalled-server-arm64+raspi.img` disk image hold the real root filesystem.
...
- `-initrd $initrd`
  - the boot loader works using `vmlinuz initrd=initrd.img root=/dev/sda2` configuration.
  - We can do without `-initrd` as the minimal kernel command line is `vmlinuz root=/dev/sda2`. Because we don't need the **initial RAM disk** to decrypt `disk`, use `raid` or other things before mount the real filesystem `/dev/sda2`.
...
qemu-system-aarch64 \
    ...
    -kernel $kernel \
    -initrd $initrd \
    -append "earlyprintk loglevel=8 root=/dev/nvme0n1p2 rootfstype=ext4 console=ttyAMA0" \
    -drive file=$img,format=raw,if=none,id=drive0 \
    -device nvme,drive=drive0,serial=deadbeaf \
    -hda $img
    -device virtio-scsi-pci,id=scsi0 \
    -drive file=$img,format=raw,if=none,id=drive1 \
