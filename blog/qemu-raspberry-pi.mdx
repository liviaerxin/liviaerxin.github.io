...
qemu-system-aarch64 \
    ...
    -device usb-ehci \
    -drive file=$img,format=raw,if=none,id=disk1 \
    -device usb-storage,drive=disk1 \
    -drive file=$img,format=raw,if=none,id=disk2 \
...
```sh
qemu-system-x86_64 -m 4G -drive file=$img,if=none,id=disk1 \
              -device ide-hd,drive=disk1,bootindex=4 \
              -drive file=$img,if=none,id=disk2 \
...
https://mgalgs.io/2012/03/23/how-to-build-a-custom-linux-kernel-for-qemu.html

```sh
kernel="$PWD/mantic-mini-iso-amd64-boot/casper/vmlinuz"
initrd="$PWD/mantic-mini-iso-amd64-boot/casper/initrd"
img="$PWD/mantic-mini-iso-amd64.iso"
